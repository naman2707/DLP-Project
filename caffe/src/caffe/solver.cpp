...
void AdaDeltaSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::CPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      // compute square of gradient in update, using local_rate
      caffe_powx(net_params[param_id]->count(),
          net_params[param_id]->cpu_diff(), Dtype(2),
          this->temp_[param_id]->mutable_cpu_data());
      
      caffe_cpu_axpby(net_params[param_id]->count(), local_rate,
          this->temp_[param_id]->cpu_data(), 1-local_rate,
          this->history_[param_id]->mutable_cpu_diff());

      // compute the difference of the past updates with the square gradient
      caffe_add_scalar(net_params[param_id]->count(),
                delta, this->update_[param_id]->mutable_cpu_data());

      caffe_add_scalar(net_params[param_id]->count(),
                delta, this->history_[param_id]->mutable_cpu_data());

      caffe_div(net_params[param_id]->count(),
                this->update_[param_id]->cpu_diff(),
                this->history_[param_id]->cpu_data(),
                this->temp_[param_id]->mutable_cpu_data());
      ...
      caffe_cpu_axpby(net_params[param_id]->count(), local_rate,
          this->temp_[param_id]->cpu_data(), 1-local_rate,
          this->update_[param_id]->mutable_cpu_diff());
    }
    break;
  case Caffe::GPU:
#ifndef CPU_ONLY
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      caffe_gpu_axpby(net_params[param_id]->count(), local_rate,
          this->temp_[param_id]->gpu_data(), 1-local_rate,
      ...
      caffe_gpu_add_scalar(net_params[param_id]->count(),
                delta, this->history_[param_id]->mutable_gpu_data());

      caffe_gpu_div(net_params[param_id]->count(),
                this->update_[param_id]->gpu_diff(),
                this->history_[param_id]->gpu_data(),
                this->temp_[param_id]->mutable_gpu_data());
      ...
      caffe_gpu_axpby(net_params[param_id]->count(), local_rate,
          this->temp_[param_id]->gpu_data(), 1-local_rate,
          this->update_[param_id]->mutable_gpu_diff());
    }
#else
    NO_GPU;
#endif
...
template <typename Dtype>
void AdaptiveLearningSolver<Dtype>::ComputeUpdateValue() {
  vector<shared_ptr<Blob<Dtype> > >& net_params = this->net_->params();
  vector<float>& net_params_lr = this->net_->params_lr();
  vector<float>& net_params_weight_decay = this->net_->params_weight_decay();
  // get the learning rate
  ...
  case Caffe::CPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      // update history
      caffe_cpu_axpby(net_params[param_id]->count(), delta,
          net_params[param_id]->cpu_diff(), Dtype(1)-delta,
          this->history_[param_id]->mutable_cpu_data());
      ...
      caffe_scal(net_params[param_id]->count(),
                local_rate, this->update_[param_id]->mutable_cpu_data());
  ...
  case Caffe::GPU:
    ...
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      // update history
      caffe_gpu_axpby(net_params[param_id]->count(), delta,
          net_params[param_id]->gpu_diff(), Dtype(1)-delta,
          this->history_[param_id]->mutable_gpu_data());
      ...
      // take min(update,1)
      caffe_gpu_add_scalar(net_params[param_id]->count(),
                Dtype(-1), this->update_[param_id]->mutable_gpu_data());
      caffe_gpu_abs(net_params[param_id]->count(),
                this->update_[param_id]->gpu_data(), 
                this->temp_[param_id]->mutable_gpu_data());
      ...
      caffe_gpu_scal(net_params[param_id]->count(),
                Dtype(.5), this->update_[param_id]->mutable_gpu_data());
      caffe_gpu_add_scalar(net_params[param_id]->count(),
                Dtype(1), this->update_[param_id]->mutable_gpu_data());

      caffe_gpu_scal(net_params[param_id]->count(),
                local_rate, this->update_[param_id]->mutable_gpu_data());
...
template <typename Dtype>
void VarianceSolver<Dtype>::ComputeUpdateValue() {
  vector<shared_ptr<Blob<Dtype> > >& net_params = this->net_->params();
  vector<float>& net_params_lr = this->net_->params_lr();
  vector<float>& net_params_weight_decay = this->net_->params_weight_decay();
  // get the learning rate
  ...
  case Caffe::CPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      caffe_mul(net_params[param_id]->count(),
                ...
                this->temp_[param_id]->mutable_cpu_data());
      caffe_cpu_axpby(net_params[param_id]->count(), delta,
                this->temp_[param_id]->cpu_data(), Dtype(1)-delta,
                this->history_[param_id]->mutable_cpu_data());
      ...
      caffe_add_scalar(net_params[param_id]->count(),
                Dtype(.01), this->temp_[param_id]->mutable_cpu_data());
      caffe_powx(net_params[param_id]->count(),
                this->temp_[param_id]->cpu_data(), Dtype(.5),
                this->temp_[param_id]->mutable_cpu_data());
      ...
      caffe_div(net_params[param_id]->count(),
                ...
                net_params[param_id]->mutable_cpu_diff());
      caffe_scal(net_params[param_id]->count(),
                local_rate, net_params[param_id]->mutable_cpu_diff());
    }
    break;
  case Caffe::GPU:
#ifndef CPU_ONLY
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      caffe_gpu_mul(net_params[param_id]->count(),
                ...
                this->temp_[param_id]->mutable_gpu_data());
      caffe_gpu_axpby(net_params[param_id]->count(), delta,
                this->temp_[param_id]->gpu_data(), Dtype(1)-delta,
                this->history_[param_id]->mutable_gpu_data());
      ...
      caffe_gpu_add_scalar(net_params[param_id]->count(),
                Dtype(.01), this->temp_[param_id]->mutable_gpu_data());
      caffe_gpu_powx(net_params[param_id]->count(),
                this->temp_[param_id]->gpu_data(), Dtype(.5),
                this->temp_[param_id]->mutable_gpu_data());
      ...
      caffe_gpu_div(net_params[param_id]->count(),
                ...
                net_params[param_id]->mutable_gpu_diff());
      caffe_gpu_scal(net_params[param_id]->count(),
                local_rate, net_params[param_id]->mutable_gpu_diff());
    }
#else
    NO_GPU;
#endif
...
INSTANTIATE_CLASS(AdaptiveLearningSolver);
INSTANTIATE_CLASS(VarianceSolver);
